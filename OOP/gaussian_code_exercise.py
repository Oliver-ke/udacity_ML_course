...
class Gaussian():
    ...
    def calculate_stdev(self, sample=True):
        ...
        for number in self.data:
            varience += pow(number - self.mean, 2)
        stdev = 0
        if sample == True:
            stdev = math.sqrt(varience / (len(self.data) - 1))
        else:
            stdev = math.sqrt(varience / len(self.data))
        self.stdev = stdev
        print(stdev, sample)
        return stdev
    ...
    def pdf(self, x):
        ...
        quocent = 1 / (self.stdev * math.sqrt(2 * math.pi))
        exponent = math.exp(- pow(x - self.mean, 2) / (2 * pow(self.stdev, 2)))
        prob_func = quocent * exponent
        return prob_func
